...
import { HassEvent} from './homeAssistant.js';
...
export const ERR_INVALID_AUTH = 'Invalid home assistant authentication';
export const ERR_CONNECTION_LOST = 'Connection with home assistant lost';
...
export class Connection {
  ...
  async connect() {
    const socket = await createSocket(this.options);
  ...
  sendMessage(message: MessageBase, commandId?: number): void {
    ...
    this.log.debug(`Sending message ${debugStringify(message)}`);
  ...
  sendMessagePromise<Result>(message: MessageBase): Promise<Result> {
    return new Promise((resolve, reject) => {
      ...
      this.log.debug(`Fetching ${CYAN}${message.type}${db}...`);
  ...
  private _handleMessage = (event: WebSocket.MessageEvent) => {
    ...
    messageGroup.forEach((message) => {
      //this.log.debug(`Received message ${debugStringify(message)}`);
      ...
      switch (message.type) {
        case 'event':
          if (info) {
            ...
            this.log.warn(`Received event for unknown subscription ${message.id}. Unsubscribing.`);
        ...
        default:
          this.log.warn(`Unhandled message: ${message}`);
      }
    });
  };
